    ...
    <div className="space-y-8 pb-12">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        ...
        <Button
          ...
          variant="outline"
          size="sm"
          className="gap-2 text-xs bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-100 dark:hover:bg-slate-800 hover:text-slate-900 dark:hover:text-white shadow-2xs"
        >
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
        {kpis.map((kpi, idx) => (
          <Card key={idx} className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800/90 text-slate-900 dark:text-slate-100 shadow-xs">
            <CardHeader className="flex flex-row items-center justify-between pb-2">
              <CardTitle className="text-xs font-semibold text-slate-500 dark:text-slate-400">{kpi.title}</CardTitle>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800/90 text-slate-900 dark:text-slate-100 shadow-xs">
          ...
          <CardContent className="h-72 pt-4">
            <ResponsiveContainer width="100%" height="100%">
              <BarChart data={metrics.jobsByCategory}>
                ...
                />
                <Bar dataKey="count" name="Jobs Posted" fill="#8b5cf6" radius={[6, 6, 0, 0]} />
              </BarChart>
            </ResponsiveContainer>
          </CardContent>
        </Card>
      ...
      <Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-800/90 text-slate-900 dark:text-slate-100 shadow-xs">
        <CardHeader className="pb-2">
          <CardTitle className="text-base font-bold text-slate-900 dark:text-white flex items-center justify-between">
            <span>Marketplace Booking Status Distribution</span>
            <span className="text-xs text-slate-500 dark:text-slate-400 font-normal">
              Total lifecycle volume: {metrics.totalBookings}
            </span>
          </CardTitle>
          <CardDescription className="text-xs text-slate-500 dark:text-slate-400">
            Current state of customer service requests
          </CardDescription>
        </CardHeader>
