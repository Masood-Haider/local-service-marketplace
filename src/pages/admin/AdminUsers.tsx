      ...
      <Card className="bg-slate-900 border-slate-800 text-slate-100 overflow-hidden">
        <CardHeader className="pb-3">
          <div className="flex items-center justify-between">
            <CardTitle className="text-base font-bold text-white">Registered Accounts</CardTitle>
        ...
        <CardContent className="p-0">
          {loading ? (
            <div className="py-20 flex flex-col items-center justify-center space-y-2">
              ...
              <p className="text-xs text-slate-400">Loading user records from Firestore...</p>
            </div>
              ...
              <Table>
                <TableHeader className="bg-slate-950/60 border-slate-800">
                  <TableRow className="border-slate-800 hover:bg-transparent">
                    <TableHead className="text-slate-400">User Profile</TableHead>
                    ...
                    <TableHead className="text-right text-slate-400">Moderation Actions</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody>
                  {filteredUsers.map((user) => (
                    <TableRow key={user.uid} className="border-slate-800/80 hover:bg-slate-800/40">
                      <TableCell>
                        <div className="flex items-center gap-3">
                          ...
                          <div>
                            <p className="font-bold text-xs text-white">{user.name || "Anonymous User"}</p>
                      ...
                      <TableCell className="text-right space-x-2">
                        <Button
                          ...
                          className={`h-7 px-2.5 text-xs ${
                            user.disabled
                              ? "text-emerald-400 border-emerald-800/40 hover:bg-emerald-950/40"
                              : "text-amber-400 border-amber-800/40 hover:bg-amber-950/40"
                          }`}
                        >
                          {user.disabled ? (
                            <>
                        ...
                        <Button
                          ...
                          className="h-7 px-2.5 text-xs text-rose-400 border-rose-900/40 hover:bg-rose-950/50"
                        >
        ...
        <DialogContent className="sm:max-w-[420px] bg-slate-900 border-slate-800 text-slate-100">
          <DialogHeader>
            <div className="flex items-center gap-2 text-rose-500 mb-1">
              ...
              <DialogTitle className="text-lg text-white">Delete User Document?</DialogTitle>
            </div>
            <DialogDescription className="text-xs text-slate-400">
              Are you sure you want to permanently remove the user profile document for{" "}
              <strong className="text-white">{deleteTarget?.name}</strong> ({deleteTarget?.email})?
